p8105\_hw6\_kb2977
================
Francois Ban
11/18/2018

### Problem 1:

``` r
homicide_data = read_csv("https://raw.githubusercontent.com/washingtonpost/data-homicides/master/homicide-data.csv")
```

    ## Parsed with column specification:
    ## cols(
    ##   uid = col_character(),
    ##   reported_date = col_integer(),
    ##   victim_last = col_character(),
    ##   victim_first = col_character(),
    ##   victim_race = col_character(),
    ##   victim_age = col_character(),
    ##   victim_sex = col_character(),
    ##   city = col_character(),
    ##   state = col_character(),
    ##   lat = col_double(),
    ##   lon = col_double(),
    ##   disposition = col_character()
    ## )

#### Part 1: Modifying and Cleaning Dataset

-   Create a city\_state variable (e.g. “Baltimore, MD”), and a binary variable indicating whether the homicide is solved. Omit cities Dallas, TX; Phoenix, AZ; and Kansas City, MO – these don’t report victim race. Also omit Tulsa, AL – this is a data entry mistake.

``` r
homicide_data = homicide_data %>% 
  mutate(city_state = str_c(city, state, sep = ", ")) %>% 
  mutate(solved = disposition) %>%
  mutate(solved = recode(solved, 
                         "Closed without arrest" = "no", 
                         "Open/No arrest" = "no", 
                         "Closed by arrest" = "yes")) %>% 
  mutate(solved = as.factor(solved))
homicide_data = homicide_data[!(homicide_data$city_state %in% c("Dallas, TX", "Phoenix, AZ", "Kansas City, MO", "Tulsa, AL")), ]
```

-   Modify victim\_race to have categories white and non-white, with white as the reference category. Be sure that victim\_age is numeric.

``` r
homicide_data = 
  homicide_data %>%
  mutate(victim_race_cat = victim_race) %>%
  mutate(victim_race_cat = recode(victim_race_cat, 
                         "Hispanic" = "non-white", 
                         "Black" = "non-white", 
                         "Other" = "non-white",
                         "Asian" = "non-white")) %>% 
  mutate(victim_race_cat = fct_infreq(victim_race_cat)) %>% 
  mutate(victim_age = as.numeric(victim_age))
```

    ## Warning in evalq(as.numeric(victim_age), <environment>): NAs introduced by
    ## coercion

#### Part 2: Baltimore, MD

-   For the city of Baltimore, MD, use the glm function to fit a logistic regression with resolved vs unresolved as the outcome and victim age, sex and race (as just defined) as predictors. Save the output of glm as an R object; apply the broom::tidy to this object; and obtain the estimate and confidence interval of the adjusted odds ratio for solving homicides comparing black victims to white victims keeping all other variables fixed.

``` r
logistic_balt = 
  homicide_data %>% 
  filter(city_state == "Baltimore, MD") %>% 
  mutate(victim_race = fct_relevel(victim_race, "White")) %>% 
  glm(solved ~ victim_age + victim_sex + victim_race, data = ., family = binomial()) %>% 
  broom::tidy() 

logistic_balt %>% 
  select(term, estimate, std.error) %>% 
  mutate(OR = exp(estimate)) %>% 
  mutate(lower_CI = exp(estimate - (1.96 * std.error)),
        upper_CI = exp(estimate + (1.96 * std.error))) %>% 
  filter(term == "victim_raceBlack") %>% 
  select(OR, lower_CI, upper_CI) %>% 
  knitr::kable(digits = 3)
```

|     OR|  lower\_CI|  upper\_CI|
|------:|----------:|----------:|
|  0.431|      0.306|      0.607|

#### Part 3: GLM for Each City

-   Now run glm for each of the cities in your dataset, and extract the adjusted odds ratio (and CI) for solving homicides comparing black victims to white victims. Do this within a “tidy” pipeline, making use of purrr::map, list columns, and unnest as necessary to create a dataframe with estimated ORs and CIs for each city.

``` r
CI_for_each_city = function(each_city){

  CI = homicide_data %>% 
  filter(city_state == each_city) %>% 
  mutate(victim_race = fct_relevel(victim_race, "White")) %>% 
  glm(solved ~ victim_age + victim_sex + victim_race, data = ., family = binomial()) %>% 
  broom::tidy() %>% 
  select(term, estimate, std.error) %>% 
  mutate(OR = exp(estimate)) %>% 
  mutate(lower_CI = exp(estimate - (1.96 * std.error)),
        upper_CI = exp(estimate + (1.96 * std.error))) %>% 
  filter(term == "victim_raceBlack") %>% 
  select(OR, lower_CI, upper_CI) 
  
CI
}
```

``` r
nest_glm_results =
  homicide_data %>% 
  group_by(city_state) %>% 
  nest() %>% 
  mutate(models = map(city_state, CI_for_each_city)) %>% 
  select(-data) %>% 
  unnest()

nest_glm_results %>% 
  knitr::kable(digits = 3)
```

| city\_state        |     OR|  lower\_CI|  upper\_CI|
|:-------------------|------:|----------:|----------:|
| Albuquerque, NM    |  0.612|      0.274|      1.364|
| Atlanta, GA        |  0.760|      0.435|      1.326|
| Baltimore, MD      |  0.431|      0.306|      0.607|
| Baton Rouge, LA    |  0.653|      0.305|      1.399|
| Birmingham, AL     |  1.052|      0.622|      1.780|
| Boston, MA         |  0.096|      0.039|      0.236|
| Buffalo, NY        |  0.368|      0.200|      0.678|
| Charlotte, NC      |  0.613|      0.351|      1.073|
| Chicago, IL        |  0.552|      0.423|      0.721|
| Cincinnati, OH     |  0.326|      0.188|      0.564|
| Columbus, OH       |  0.859|      0.636|      1.161|
| Denver, CO         |  0.581|      0.324|      1.043|
| Detroit, MI        |  0.652|      0.488|      0.871|
| Durham, NC         |  1.087|      0.434|      2.722|
| Fort Worth, TX     |  0.931|      0.597|      1.452|
| Fresno, CA         |  0.424|      0.206|      0.874|
| Houston, TX        |  0.951|      0.754|      1.198|
| Indianapolis, IN   |  0.502|      0.379|      0.666|
| Jacksonville, FL   |  0.646|      0.492|      0.847|
| Las Vegas, NV      |  0.777|      0.583|      1.037|
| Long Beach, CA     |  0.997|      0.466|      2.134|
| Los Angeles, CA    |  0.610|      0.437|      0.853|
| Louisville, KY     |  0.392|      0.259|      0.595|
| Memphis, TN        |  0.804|      0.537|      1.202|
| Miami, FL          |  0.574|      0.373|      0.884|
| Milwaukee, wI      |  0.592|      0.376|      0.930|
| Minneapolis, MN    |  0.640|      0.339|      1.210|
| Nashville, TN      |  0.873|      0.633|      1.203|
| New Orleans, LA    |  0.451|      0.285|      0.714|
| New York, NY       |  0.439|      0.227|      0.848|
| Oakland, CA        |  0.186|      0.091|      0.380|
| Oklahoma City, OK  |  0.738|      0.505|      1.079|
| Omaha, NE          |  0.134|      0.073|      0.244|
| Philadelphia, PA   |  0.641|      0.483|      0.851|
| Pittsburgh, PA     |  0.279|      0.159|      0.488|
| Richmond, VA       |  0.490|      0.177|      1.359|
| San Antonio, TX    |  0.735|      0.456|      1.186|
| Sacramento, CA     |  0.753|      0.413|      1.373|
| Savannah, GA       |  0.567|      0.266|      1.210|
| San Bernardino, CA |  0.875|      0.375|      2.044|
| San Diego, CA      |  0.472|      0.274|      0.813|
| San Francisco, CA  |  0.407|      0.252|      0.656|
| St. Louis, MO      |  0.574|      0.404|      0.817|
| Stockton, CA       |  0.459|      0.228|      0.923|
| Tampa, FL          |  1.258|      0.626|      2.528|
| Tulsa, OK          |  0.596|      0.402|      0.882|
| Washington, DC     |  0.506|      0.255|      1.003|

#### Part 4: Plot for Each City

\*Create a plot that shows the estimated ORs and CIs for each city. Organize cities according to estimated OR, and comment on the plot.

``` r
OR_plot = nest_glm_results %>%
  ggplot(aes(x = city_state, y = OR)) + 
    geom_point(aes(reorder(city_state, OR)), size = 0.7, color = "BLUE") +
    geom_errorbar(aes(ymin = lower_CI, ymax = upper_CI), 
                  color = "RED", 
                  width = 0.1, 
                  alpha = 0.5) +
    labs(title = "Estimated Odds Ratio of Solved Homicides Comparing Black Victims to White Victimes in Each City",
        x = "City",
        y = "Estimated OR") + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7))
OR_plot
```

![](p8105_hw6_kb2977_files/figure-markdown_github/OR%20plots-1.png)

\*The odd ratios of solving homicides for black victims to white victims is highest for Tampa, FL and lowest for Boston, MA. Only about three cities (Birmingham, AL, Durham, NC, and Tampa, FL) have a odds ratio of greater than 1. The rest have an odds ratio of less than 1. This means that homicides in which the victim is black are substantially less likely to be resolved that those in which the victim is white Long Beach, CA seems to have an OR around 1 meaning the odds of solving homicides for black victims is similar, if not the same, to the odds of solving homicides for white victims in Long Beach, CA.

### Problem 2:

#### Part 1: Cleaning Data

-   Load and clean the data for regression analysis (i.e. convert numeric to factor where appropriate, check for missing data, etc.)

``` r
birthweight_data = 
  read_csv(file = "./data/birthweight.csv") %>% 
  janitor::clean_names() %>% 
  mutate(frace = as.factor(frace),
         mrace = as.factor(mrace),
         malform = as.factor(malform),
         babysex = as.factor(babysex)) %>% 
  na.omit()
```

    ## Parsed with column specification:
    ## cols(
    ##   .default = col_integer(),
    ##   gaweeks = col_double(),
    ##   ppbmi = col_double(),
    ##   smoken = col_double()
    ## )

    ## See spec(...) for full column specifications.

``` r
birthweight_data[!complete.cases(birthweight_data), ] #no missing data!
```

    ## # A tibble: 0 x 20
    ## # ... with 20 variables: babysex <fct>, bhead <int>, blength <int>,
    ## #   bwt <int>, delwt <int>, fincome <int>, frace <fct>, gaweeks <dbl>,
    ## #   malform <fct>, menarche <int>, mheight <int>, momage <int>,
    ## #   mrace <fct>, parity <int>, pnumlbw <int>, pnumsga <int>, ppbmi <dbl>,
    ## #   ppwt <int>, smoken <dbl>, wtgain <int>

This dataset consists of 4342 observations and 20 variables. The following variables are in this dataset: babysex, bhead, blength, bwt, delwt, fincome, frace, gaweeks, malform, menarche, mheight, momage, mrace, parity, pnumlbw, pnumsga, ppbmi, ppwt, smoken, wtgain. The dataset was cleaned and four variables were changed to factor variables.

#### Part 2: Propose a regression model for birthweight.

-   Below, I developed a fitted model for predicting birthweight using mother's weight at delivery. I predicted that there is a positive linear relationship between mother's weight and baby's birthweight. I included the variables malform (presence of malformations that could affect weight) and fincome (family monthly income) as covariates as I believe that these could affect the relationship between our exposure and outcome.

``` r
fit_model_1 = lm(bwt ~ delwt + malform + fincome, data = birthweight_data) 

##First, I checked to make sure the residuals for the variable delwt were centered around zero 
birthweight_data %>% 
  add_residuals(fit_model_1) %>%
  ggplot(aes(x = delwt, y = resid)) + 
    geom_violin() +
    labs(title = "Residuals vs. Mother's Weight",
         x = "Mother's Weight at Delivery in Pounds", 
         y = "Residuals")
```

![](p8105_hw6_kb2977_files/figure-markdown_github/fitted%20model%201-1.png)

``` r
##Then, I checked for linearity between delwt (mother's weight) and bwt (birthweight)
birthweight_data %>% 
  ggplot(aes(x = delwt, y = bwt)) + 
    geom_point(alpha = 0.5) + 
    geom_smooth() +
    labs(title = "Birthweight vs. Mother's Weight",
         x = "Mother's Weight at Delivery in Pounds", 
         y = "Infant Birthweight in Grams")
```

    ## `geom_smooth()` using method = 'gam' and formula 'y ~ s(x, bs = "cs")'

![](p8105_hw6_kb2977_files/figure-markdown_github/fitted%20model%201-2.png)

``` r
##Lastly, I plotted predicted values of infant birthweight (bwt) and residuals to examine if there is constant variance
birthweight_data %>% 
  add_residuals(fit_model_1) %>% 
  add_predictions(fit_model_1) %>% 
  ggplot(aes(x = pred, y = resid)) +
    geom_point(alpha = 0.5) +
    stat_smooth(method = "lm") +
    labs(title = "Residuals vs. Predicted Birthweight",
        x = "Fitted Values for Infant Birthweight",
        y = "Residuals")
```

![](p8105_hw6_kb2977_files/figure-markdown_github/fitted%20model%201-3.png)

Given the plots above, the residuals appear to be centered around 0 with no clear pattern. Most of the predicted data points are around 3000-3200 grams, however, the residuals are very spread in that they range from more than -2500 to more than 1500. This could mean that this model isn't the best at predicting birthweight. Furthermore, when examining the linear plot between mother's weight at delivery and infant birthweight, the relationship, although positive, does not seem extremely linear as the smooth line fitted to this plot is not straight but curved. It is evident from this plot that there is one observation, 334 for mother's weight at delivery in pounds, that seems to be an outlier.

#### Part 3: Compare your model to two others

-   One using length at birth and gestational age as predictors (main effects only)

``` r
fit_model_2 = lm(bwt ~ gaweeks + blength, data = birthweight_data)
```

-   One using head circumference, length, sex, and all interactions (including the three-way interaction) between these

``` r
fit_model_3 = lm(bwt ~ bhead + blength + babysex + bhead*blength + blength*babysex + bhead*babysex + bhead*blength*babysex, data = birthweight_data)
```

-   Make this comparison in terms of the cross-validated prediction error; use crossv\_mc and functions in purrr as appropriate.

``` r
cv_df =
  modelr::crossv_mc(birthweight_data, 100)
cv_df %>% pull(train) %>% .[[1]] %>% as_tibble
```

    ## # A tibble: 3,473 x 20
    ##    babysex bhead blength   bwt delwt fincome frace gaweeks malform menarche
    ##    <fct>   <int>   <int> <int> <int>   <int> <fct>   <dbl> <fct>      <int>
    ##  1 1          34      48  3062   156      65 2        25.9 0             14
    ##  2 2          36      50  3345   148      85 1        39.9 0             12
    ##  3 1          34      52  3062   157      55 1        40   0             14
    ##  4 2          34      52  3374   156       5 1        41.6 0             13
    ##  5 1          33      52  3374   129      55 1        40.7 0             12
    ##  6 2          33      46  2523   126      96 2        40.3 0             14
    ##  7 2          33      49  2778   140       5 1        37.4 0             12
    ##  8 1          36      52  3515   146      85 1        40.3 0             11
    ##  9 1          33      50  3459   169      75 2        40.7 0             12
    ## 10 1          35      51  3459   146      55 1        39.4 0             12
    ## # ... with 3,463 more rows, and 10 more variables: mheight <int>,
    ## #   momage <int>, mrace <fct>, parity <int>, pnumlbw <int>, pnumsga <int>,
    ## #   ppbmi <dbl>, ppwt <int>, smoken <dbl>, wtgain <int>

``` r
cv_df %>% pull(test) %>% .[[1]] %>% as_tibble
```

    ## # A tibble: 869 x 20
    ##    babysex bhead blength   bwt delwt fincome frace gaweeks malform menarche
    ##    <fct>   <int>   <int> <int> <int>   <int> <fct>   <dbl> <fct>      <int>
    ##  1 2          34      51  3629   177      35 1        39.9 0             13
    ##  2 2          35      51  3317   130      55 1        43.4 0             13
    ##  3 1          35      51  3544   129      65 1        39.6 0             12
    ##  4 2          36      52  3629   154      65 1        40.3 0             11
    ##  5 2          35      57  3374   147      45 1        39.6 0             12
    ##  6 1          34      49  3175   148      96 2        39.7 0             10
    ##  7 2          36      51  2977   135      45 1        41.7 0             13
    ##  8 1          36      52  3629   152      45 1        39.6 0             11
    ##  9 2          34      49  3118   161      45 2        38.9 0             10
    ## 10 2          35      53  3600   141      35 1        42.3 0             14
    ## # ... with 859 more rows, and 10 more variables: mheight <int>,
    ## #   momage <int>, mrace <fct>, parity <int>, pnumlbw <int>, pnumsga <int>,
    ## #   ppbmi <dbl>, ppwt <int>, smoken <dbl>, wtgain <int>

``` r
cv_df =
  cv_df %>% 
  mutate(train = map(train, as_tibble),
         test = map(test, as_tibble))
```

``` r
cv_df = 
  cv_df %>% 
  mutate(model_1 = map(train, ~lm(bwt ~ gaweeks, data = birthweight_data)),
         model_2 = map(train, ~lm(bwt ~ gaweeks + blength, data = birthweight_data))) %>% 
  mutate(rmse_model_1 = map2_dbl(model_1, test, ~rmse(model = .x, data = birthweight_data)),
         rmse_model_2 = map2_dbl(model_2, test, ~rmse(model = .x, data = birthweight_data)))

cv_df %>% 
  select(starts_with("rmse")) %>% 
  gather(key = model, value = rmse) %>% 
  mutate(model = str_replace(model, "rmse_", ""),
         model = fct_inorder(model)) %>% 
  ggplot(aes(x = model, y = rmse)) + geom_violin()
```

![](p8105_hw6_kb2977_files/figure-markdown_github/unnamed-chunk-1-1.png)

Note that although we expect your model to be reasonable, model building itself is not a main idea of the course and we don’t necessarily expect your model to be “optimal”.
